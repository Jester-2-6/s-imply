...
def run_benchmark_for_circuit(bench_path: str, timeout_per_fault: float = 10.0):
    ...
    print(
        f"Finished {os.path.basename(bench_path)}: "
        f"FC={(detected_count/total_faults*100):.2f}%, "
        f"Time={total_time:.2f}s, Avg={avg_time_ms:.2f}ms"
    )
...
def main():
    ...
    print("Starting Parallel Vanilla ISCAS85 Benchmark with 10s timeout per fault...")
    start_all = time.time()

    all_paths = [
        os.path.join(bench_dir, bf)
        for bf in bench_files
        if os.path.exists(os.path.join(bench_dir, bf))
    ]
