...
from src.atpg.podem import podem
...
def run_classic_podem(bench_path: str, fault: Fault) -> tuple[bool, float]:
...
def main():
    ...
    parser.add_argument("--gate", type=int, default=259, help="Fault gate ID")
    ...
    for r in range(args.repeats):
        ok_c, t_c = run_classic_podem(args.bench, fault)
        classic_times.append(t_c)
        classic_ok.append(ok_c)

        ok_a, t_a = run_ai_podem(args.bench, fault, args.bench, device, args.model)
        ai_times.append(t_a)
        ai_ok.append(ok_a)

        print(
            f"  Run {r + 1}/{args.repeats}"
            f"  |  Classic PODEM : {'OK  ' if ok_c else 'FAIL'}  {t_c:.4f}s"
            f"  |  AI-PODEM : {'OK  ' if ok_a else 'FAIL'}  {t_a:.4f}s"
        )
    ...
    print()
    print("=" * 60)
    print(f"{'Method':<20} {'Pass':>6} {'Avg time (s)':>14}  {'vs Classic':>10}")
    print("-" * 60)
    print(f"{'Classic PODEM':<20} {c_pass}/{args.repeats:>3} {avg_c:>14.4f}  {'1.00×':>10}")
    print(f"{'AI-PODEM':<20} {a_pass}/{args.repeats:>3} {avg_a:>14.4f}  {speedup:>9.2f}×")
    print("=" * 60)
