...
from src.atpg.podem import podem, get_all_faults, initialize
...
def main():
    ...
    hard_faults, solved_faults, unsolvable_faults = find_hard_faults(
        circuit_path, 
        timeout_seconds
    )
    ...
    print("SUMMARY")
    print("="*80)
    print(f"Total faults tested: {len(hard_faults) + len(solved_faults) + len(unsolvable_faults)}")
    ...
    if solved_faults:
        ...
        print(f"\n{'='*80}")
        print(f"TOP 10 SLOWEST SOLVED FAULTS:")
        print("="*80)
