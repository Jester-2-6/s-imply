...
def diagnose(args):
    ...
    print(
        f"Model config: dim={model_dim}, nhead={nhead}, enc={enc_layers}, "
        f"int={int_layers}, ffn={ffn_dim}"
    )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            for b in range(B):
                ...
                avg_path_len = (
                    path_len[b][path_valid[b]].float().mean().item() if path_valid[b].any() else 0
                )
    ...
    print(
        f"Fully valid (acc): {fully_valid}/{total_samples} = "
        f"{fully_valid/max(1,total_samples):.3f}"
    )
    print(f"SAT samples: {sat_count}")
    print(
        f"Reconv failures: {reconv_failures}/{total_samples} = "
        f"{reconv_failures/max(1,total_samples):.3f}"
    )
    print(
        f"Trivial (all-0 or all-1): {trivial_count}/{total_samples} = "
        f"{trivial_count/max(1,total_samples):.3f}"
    )
    print(
        f"Edge accuracy: {(total_edges - total_wrong_edges)}/{total_edges} = "
        f"{1 - total_wrong_edges/max(1,total_edges):.4f}"
    )
    ...
    for v in sorted(pred_value_counts.keys()):
        print(
            f"  Value {v}: {pred_value_counts[v]}/{total_preds} = "
            f"{pred_value_counts[v]/max(1,total_preds):.3f}"
        )
    ...
    print(
        f"  Zero-error samples: {(err_arr == 0).sum()}/{total_samples} = "
        f"{(err_arr==0).sum()/total_samples:.3f}"
    )
    for threshold in [0, 1, 2, 3, 5, 10, 20]:
        count = (err_arr <= threshold).sum()
        print(f"  ≤{threshold} errors: {count}/{total_samples} = " f"{count/total_samples:.3f}")

    print("\n--- Edges Checked per Sample ---")
    chk_arr = np.array(edges_checked_per_sample)
    print(
        f"  Mean: {chk_arr.mean():.1f}, Median: {np.median(chk_arr):.1f}, " f"Max: {chk_arr.max()}"
    )
    ...
    for gt_val in sorted(GATE_NAMES.keys()):
        ...
        if total > 0:
            rate = wrong / total
            print(f"  {GATE_NAMES[gt_val]:6s}: {wrong:7d}/{total:7d} = " f"{rate:.4f} error rate")
    ...
    for pl in sorted(path_len_errors.keys()):
        ...
        if len(errs) >= 5:  # Only show bins with enough samples
            ...
            print(
                f"  PathLen ~{pl:3d}: n={len(errs):5d}, mean_errors={mean_err:.2f}, "
                f"zero_err_frac={zero_frac:.3f}"
            )
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                for b in range(B):
                    if local_wrong[b].item() > 0 and fail_count < 10:
                        ...
                        print(
                            f"\n  Sample [{batch_idx}:{b}] — "
                            f"{local_wrong[b].item()} edge errors"
                        )
                            ...
                            if path_errs > 0:
                                ...
                                for pos in range(int(pl) - 1):
                                    if wrong_edges[b, p, pos].item():
                                        ...
                                        prob0 = torch.softmax(logits[b, p, pos + 1], dim=-1)[
                                            0
                                        ].item()
                                        prob1 = 1 - prob0
                                        print(
                                            f"      pos {pos}->{pos+1}: "
                                            f"node {nid_prev}->{nid_cur} "
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--data-dir",
        default="data/datasets/shards_anchored",
