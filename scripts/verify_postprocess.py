...
from src.ml.train_reconv import TrainConfig, make_dataloaders, resolve_gate_types
...
def main():
    ...
    cfg = TrainConfig(
        dataset='', output='', processed_dir=args.data_dir,
        batch_size=args.batch_size, model_dim=model_dim, nhead=nhead,
        num_encoder_layers=enc_layers, num_interaction_layers=int_layers,
    ...
    model = MultiPathTransformer(
        input_dim=emb_dim, model_dim=model_dim, nhead=nhead,
        num_encoder_layers=enc_layers, num_interaction_layers=int_layers,
        dim_feedforward=ffn_dim,
    ).to(device)
    ...
    # Metrics
    before = {'samples': 0, 'zero_err': 0, 'total_wrong': 0, 'total_edges': 0, 'reconv_fail': 0}
    after  = {'samples': 0, 'zero_err': 0, 'total_wrong': 0, 'total_edges': 0, 'reconv_fail': 0}
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            logits, _ = model(paths, masks, gate_types=gtypes if cfg.use_gate_type_embedding else None)
    ...
    def pct(n, d): return f"{n/max(1,d):.4f}" if d > 0 else "N/A"
    
    b_acc = before['zero_err'] / max(1, before['samples'])
    a_acc = after['zero_err'] / max(1, after['samples'])
    print(f"{'Zero-error rate (acc)':<30s}  {pct(before['zero_err'], before['samples']):>12s}  {pct(after['zero_err'], after['samples']):>12s}  {a_acc - b_acc:>+10.4f}")
    ...
    print(f"{'Total edge errors':<30s}  {before['total_wrong']:>12d}  {after['total_wrong']:>12d}  {after['total_wrong'] - before['total_wrong']:>+10d}")
    print(f"{'Reconv failures':<30s}  {before['reconv_fail']:>12d}  {after['reconv_fail']:>12d}  {after['reconv_fail'] - before['reconv_fail']:>+10d}")
