...
def verify_inversions(dataset_path, checkpoint_path, num_samples=100):
    ...
    loader = DataLoader(
        dataset, batch_size=1, shuffle=True, collate_fn=reconv_collate, num_workers=0
    )
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            P, _L = node_ids.shape[1], node_ids.shape[2]
            ...
            for p in range(P):
                ...
                for idx_l in range(valid_len):
                    nid = int(node_ids[b, p, idx_l].item())
                    val = int(actions[b, p, idx_l].item())
    ...
    inv_rate = (inversion_failures / max(1, inversion_checks)) * 100
    print(f"Inversion Failures: {inversion_failures} ({inv_rate:.2f}%)")
    print(f"Buffer Logic Checks (BUFF/OR/AND): {buffer_checks}")
    buf_rate = (buffer_failures / max(1, buffer_checks)) * 100
    print(f"Buffer Failures: {buffer_failures} ({buf_rate:.2f}%)")

    print("\n--- Breakdown ---")
    for gt, s in stats.items():
        if s["checks"] > 0:
            rate = (s["fails"] / s["checks"]) * 100
            print(f"{gt.name}: {s['fails']}/{s['checks']} ({rate:.1f}%)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num-samples", type=int, default=1000, help="Total gate checks to perform"
    )
