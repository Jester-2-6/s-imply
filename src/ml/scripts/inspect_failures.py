...
def inspect_failures(dataset_path, checkpoint_path, batch_size=1, num_workers=0):
    ...
    dataset = ReconvergentPathsDataset(
        dataset_path, device=torch.device("cpu"), prefer_value=1, anchor_in_dataset=True
    )
    loader = DataLoader(
        dataset,
        ...
        for i, batch in enumerate(loader):
            ...
            if i % 10 == 0:
                ...
                if curr_total > 0:
                    ...
                    curr_reconv = (
                        sum(x["reconv_match_rate"] for x in successes + failures) / curr_total
                    )
                    print(
                        f"Processed {i} batches ({curr_total} samples)... "
                        f"Acc: {curr_acc:.4f} Edge: {curr_edge:.4f} "
                        f"Reconv: {curr_reconv:.4f}"
                    )
    ...
    print("\nExample Failures:")
    for f in failures[:5]:
        print(f"Index: {f['index']}, Edge Acc: {f['edge_acc']:.2f}, " f"Gates: {f['gate_types']}")
