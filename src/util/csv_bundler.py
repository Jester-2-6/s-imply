...
def bundle_csvs_to_excel(
    ...
    try:
        ...
        raise RuntimeError(
            "pandas is required for bundle_csvs_to_excel. "
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bundle all CSV files in a directory into a single Excel "
        "workbook (each CSV -> one sheet)."
    )
    parser.add_argument("directory", help="Directory containing CSV files")
    parser.add_argument(
        "output",
        nargs="?",
        help="Output Excel file path (.xlsx). If omitted uses "
        "<directory_name>.xlsx in the parent directory.",
    )
